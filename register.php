...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $password_confirm) {
        $message = '<span class="blink">Password dan konfirmasi password tidak sama.</span>';
    } else {
        // Cek apakah username sudah ada
        $result = $koneksi->query("SELECT * FROM users WHERE username='$username'");
        if ($result->num_rows > 0) {
            $message = '<span class="blink">Username sudah digunakan.</span>';
        } else {
            // Hash password dan simpan data user baru
